...
#SBATCH --nodes=1
#SBATCH --job-name=nvr_lacr_llm-eval.squad
...
echo "Evaluating $model_name at step $step on SQuAD"
python -m eval.squad.run_eval \
    --model_name_or_path models/hf_models/$model_name \
    --output_dir results/squad-ice${num_incontext_examples}/$model_name/$step \
