...
def evaluate_repeat_copy_logic(model, tokenizer, test_df, batch_size, num_incontext_examples, qa_format):
    ...
    print(f"--- BB Repeat Copy Logic example prompt ---\n{prompts[0]}\n----------------------")
    ...
    results = []
    for prompt, output, answer in zip(prompts, outputs, test_df.target):
        output = output.split("\n\n")[0]
        results.append(
            {
                "prompt": prompt,
...
def main(
    ...
    test_df = pd.DataFrame(read_json("olmo_data/eval/bigbench-repeat-copy-logic/task.json")["examples"])

    if max_num_examples:
        test_df = test_df.sample(min(len(test_df), max_num_examples))

    results = evaluate_repeat_copy_logic(
        model,
        tokenizer,
        test_df,
