...
def evaluate_copa(model, tokenizer, test_df, batch_size):
    prompts = []
    for _, row in test_df.iterrows():
        prompt = row["premise"].strip() + f" What was the {row['question']} of this?\n"
        prompt += f"A. {row['choice1']}\n"
        prompt += f"B. {row['choice2']}\n\n"
        prompt += "Answer"
...
def main(
    ...
    results = evaluate_copa(model, tokenizer, test_df, batch_size=eval_batch_size)
    metrics = {
